      ...
      if (!repoExists) {
        try {
          await createRepo({
            ...
            private: isPrivate,
            license: 'mit',
            description: project.description || 'Dataset created with 数据预处理'
          });
...
async function uploadFile(token, datasetName, filePath, destFileName) {
  try {
    ...
    await uploadFiles({
      ...
      commitDescription: `Files uploaded using 数据预处理`
    });
...
## Description
${projectDescription || 'This dataset was created using the 数据预处理 tool.'}

## Format
This dataset is in ${formatType} format.

## Creation Method
This dataset was created using the 数据预处理 tool.

> 数据预处理 is a specialized application designed to streamline the creation of fine-tuning datasets for Large Language Models (LLMs). It offers an intuitive interface for uploading domain-specific files, intelligently splitting content, generating questions, and producing high-quality training data for model fine-tuning.
